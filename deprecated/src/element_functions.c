...
void initialize_ELEMENTs(void)
{
	...
	*(bool*)&ELEMENT->tensor_product = false;
	...
	ELEMENT->VeEcon[3*NEVEMAX  ] = 0; ELEMENT->VeEcon[3*NEVEMAX+1] = 3;
...
// Make into a separate function for initializing ELEMENTs to make this clearner (ToBeDeleted).
	...
	*(bool*)&ELEMENT->tensor_product = false;
	...
	ELEMENT->VeFcon[4*NFVEMAX  ] = 0; ELEMENT->VeFcon[4*NFVEMAX+1] = 1; ELEMENT->VeFcon[4*NFVEMAX+2] = 2; ELEMENT->VeFcon[4*NFVEMAX+3] = 3;
	ELEMENT->VeEcon[0*NEVEMAX  ] = 0; ELEMENT->VeEcon[0*NEVEMAX+1] = 2;
	ELEMENT->VeEcon[1*NEVEMAX  ] = 1; ELEMENT->VeEcon[1*NEVEMAX+1] = 3;
	ELEMENT->VeEcon[2*NEVEMAX  ] = 0; ELEMENT->VeEcon[2*NEVEMAX+1] = 1;
	ELEMENT->VeEcon[3*NEVEMAX  ] = 2; ELEMENT->VeEcon[3*NEVEMAX+1] = 3;
