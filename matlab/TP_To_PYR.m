...
elseif (~isempty(strfind(NodeType,'GJ')));     PMax = 6;
end
...
% Note: ((1-c)/2)^2 scaling from transformation of ref PYR to HEX
%       1/sqrt(2) scaling from height of ref PYR
% For GJ (alp = 2.0, bet = 0.0), weights already include (1-c)^2 term.
if ((P > 1 && ~isempty(strfind(NodeType,'GLL'))) || ...
    (P > 0 && ~isempty(strfind(NodeType,'GL'))))
    w = wTP*1/sqrt(2).*((1.0-abc(:,3))/2).^2;
elseif (~isempty(strfind(NodeType,'WVHToP')))
    w = 4/3*wTP*1/sqrt(2).*((1.0-abc(:,3))/2).^2;
elseif (P > 0 && ~isempty(strfind(NodeType,'GJ')))
%     w = wTP*1/sqrt(2).*((1.0-abc(:,3))/2).^2;
    w = 4/3*wTP*1/sqrt(2).*(1/2).^2;
end
% sum(w)
