...
void array_sort_i(int NRows, int NCols, int *A, int *Indices, char ordering, char trans) {
  ...
  for (row = 0; row < NRows; row++) {
    if (row == 0) {
      ...
      for (i = 0; i < SortLen; i++) Indicestmp[i] = i;
      ...
      colMs[0] = 0; colMe[row-1] = 0;
      while (colMe[row-1] < NCols-1) {
        for (rowM = 0; rowM < row; rowM++) {
          if (rowM > 0) colMs[rowM] = colMs[rowM-1];
          col = colMs[rowM]+1;
          while (col < NCols && A[rowM*NCols+col] == A[rowM*NCols+colMs[rowM]]) col++; 
          colMe[rowM] = col-1;
        }
        rowM = rowM-1;
        ...
        for (rowS = row+1; rowS < NRows; rowS++) {
          count = 0;
          for (col = colMs[rowM]; col <= colMe[rowM]; col++) { 
