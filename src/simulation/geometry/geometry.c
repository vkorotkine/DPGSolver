...
/** \brief Pointer to functions computing \ref Solver_Volume::geom_coef.
...
void compute_geometry_volume (struct Solver_Volume* s_vol, const struct Simulation *sim)
{
	...
	compute_geom_coef_fptr compute_geom_coef = set_fptr_geom_coef(sim->domain_type,vol->curved);
	compute_geom_coef(sim,s_vol);

	const int d = ((struct const_Element*)g_e)->d;

	const struct const_Multiarray_d*const geom_coef = s_vol->geom_coef;
	...
	compute_detJV((struct const_Multiarray_d*)jacobian_vc,(struct Multiarray_d*)s_vol->jacobian_det_vc);
	compute_cofactors((struct const_Multiarray_d*)jacobian_vm,(struct Multiarray_d*)s_vol->metrics_vm);

	destructor_Multiarray_d(jacobian_vm);
	destructor_Multiarray_d(jacobian_vc);

	const struct const_Multiarray_d* met_vm = s_vol->metrics_vm;

	resize_Multiarray_d((struct Multiarray_d*)s_vol->metrics_vc,3,(ptrdiff_t[]){n_vc,d,d});
...
static void compute_geometry_face (struct Simulation *sim, struct Solver_Face* face)
{
	...
	compute_unit_normals_and_det(ind_lf,e->normals,metrics_fc,
...
static void compute_geom_coef_curved (const struct Simulation*const sim, struct Solver_Volume*const s_vol)
{
	UNUSED(sim);
	struct Volume* vol = (struct Volume*) s_vol;
	const struct Geometry_Element* g_e = (struct Geometry_Element*) vol->element;
...
// Potentially not the correct operator. vc0_vgs_vgc? vc0_vvs_vgc?
	...
	const struct const_Multiarray_d* geom_coef =
		constructor_mm_NN1_Operator_const_Multiarray_d(vc0_vg_vg,vol->xyz_ve,'C','d',2,NULL); // keep
EXIT_ERROR("Add support after output to paraview is working.");

	destructor_const_Multiarray_d(s_vol->geom_coef);
	const_constructor_move_const_Multiarray_d(&s_vol->geom_coef,geom_coef);
}
