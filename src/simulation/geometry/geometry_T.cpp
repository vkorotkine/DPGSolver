...
if (1) {
	...
	} ops = { .cppv1_vg_vm = set_MO_from_MO(g_e->cv1_vg_vm[curved],1,(ptrdiff_t[]){0,0,p_g,p_g}),
	          .cppv1_vg_vc = set_MO_from_MO(g_e->cv1_vg_vc[curved],1,(ptrdiff_t[]){0,0,p,p_g}),
	          .vv0_vm_vc = get_Multiarray_Operator(g_e->vv0_vm_vc[curved],(ptrdiff_t[]){0,0,p,p_g}), };

	const ptrdiff_t n_vm = ops.cppv1_vg_vm.data[0]->op_std->ext_0,
	                n_vc = ops.cppv1_vg_vc.data[0]->op_std->ext_0;
	...
	for (ptrdiff_t row = 0; row < d; ++row) {
		mm_NN1C_Operator_Multiarray_T(ops.cppv1_vg_vm.data[row],geom_coef,jacobian_vm,op_format,2,NULL,&row);
		mm_NN1C_Operator_Multiarray_T(ops.cppv1_vg_vc.data[row],geom_coef,jacobian_vc,op_format,2,NULL,&row);
	}
...
void compute_geometry_face_T (struct Solver_Face_T* s_face, const struct Simulation*const sim)
{
	...
	if (!vol->curved) {
		ops.cppv0_vg_fc = get_Multiarray_Operator(g_e->cv0_vgs_fc[curved_f],(ptrdiff_t[]){ind_lf,0,0,p_f,1});
		ops.vv0_vm_fc = get_Multiarray_Operator(g_e->vv0_vms_fc[curved_f],(ptrdiff_t[]){ind_lf,0,0,p_f,1});
	} else {
		ops.cppv0_vg_fc = get_Multiarray_Operator(g_e->cv0_vgc_fc[curved_f],(ptrdiff_t[]){ind_lf,0,0,p_f,p_v});
...
static void constructor_volume_metric_terms_T (const char node_type, struct Solver_Volume_T*const s_vol)
{
	...
	const ptrdiff_t n_vX = con.cppv1_vg_vX->data[0]->op_std->ext_0;
	struct Multiarray_T*const jacobian_vX = constructor_empty_Multiarray_T('C',3,(ptrdiff_t[]){n_vX,DIM,DIM}); // d.

	const char op_format = get_set_op_format(0);
	const struct const_Multiarray_T*const geom_coef = s_vol->geom_coef;
	for (ptrdiff_t row = 0; row < DIM; ++row)
		mm_NN1C_Operator_Multiarray_T(con.cppv1_vg_vX->data[row],geom_coef,jacobian_vX,op_format,2,NULL,&row);
