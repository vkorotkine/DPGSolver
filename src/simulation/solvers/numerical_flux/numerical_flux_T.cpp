...
struct Numerical_Flux_Input_T* constructor_Numerical_Flux_Input_T (const struct Simulation* sim)
{
	...
	case 'e':
		num_flux_i->bv_l.cppompute_member = test_case->boundary_value_comp_mem_e;
		...
		break;
	case 'i':
		num_flux_i->bv_l.cppompute_member = test_case->boundary_value_comp_mem_i;
...
struct Numerical_Flux_T* constructor_Numerical_Flux_T (const struct Numerical_Flux_Input_T* num_flux_i)
{
	...
	/// const bool* c_m = num_flux_i->flux_i.cppompute_member;
	const bool* c_m = num_flux_i->bv_l.cppompute_member;
...
{
	if (num_flux_i->bv_l.cppompute_member[1] != true || num_flux_i->bv_r.ds_ds == NULL)
		return;
...
static void combine_num_flux_boundary_dnnf_dg_g_T
	(struct Numerical_Flux_Input_T*const num_flux_i, struct mutable_Numerical_Flux_T*const num_flux)
{
	if (num_flux_i->bv_l.cppompute_member[3] != true || num_flux_i->bv_r.dg_dg == NULL)
		return;
...
static void combine_num_flux_boundary_dnnf_dg_s_T
	(struct Numerical_Flux_Input_T*const num_flux_i, struct mutable_Numerical_Flux_T*const num_flux)
{
	if (num_flux_i->bv_l.cppompute_member[4] != true || num_flux_i->bv_r.dg_ds == NULL)
		return;
