	...
	(const struct Simulation* sim, struct Solver_Storage_Implicit* ssi, struct Intrusive_List* volumes)
{
	...
	for (struct Intrusive_Link* curr = volumes->first; curr; curr = curr->next) {
		...
		s_params.cppompute_rlhs(&s_params,flux_i,dpg_s_vol,ssi,sim,sim_c);
	}
...
static struct S_Params_DPG set_s_params_dpg (const struct Simulation* sim)
{
	...
	case 'i':
		if (test_case->has_1st_order && !test_case->has_2nd_order)
			s_params.cppompute_rlhs = compute_rlhs_1;
		else if (!test_case->has_1st_order && test_case->has_2nd_order)
			EXIT_ADD_SUPPORT; //s_params.cppompute_rlhs = compute_rlhs_2;
		else if (test_case->has_1st_order && test_case->has_2nd_order)
			EXIT_ADD_SUPPORT; //s_params.cppompute_rlhs = compute_rlhs_12;
		else
	...
	switch (test_case->ind_test_norm) {
		case TEST_NORM_H0:        s_params.cpponstructor_norm_DPG = constructor_norm_DPG__h0;        break;
		case TEST_NORM_H1:        s_params.cpponstructor_norm_DPG = constructor_norm_DPG__h1;        break;
		case TEST_NORM_H1_UPWIND: s_params.cpponstructor_norm_DPG = constructor_norm_DPG__h1_upwind; break;
		case TEST_NORM_ADJOINT:   s_params.cpponstructor_norm_DPG = constructor_norm_DPG__adjoint;   break;
