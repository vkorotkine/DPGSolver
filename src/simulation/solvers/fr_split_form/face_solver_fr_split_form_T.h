...
/** \file
 *  \brief Provides the interface for the templated \ref FRSF_Solver_Face_T container and associated functions.
 *
...
/// \brief Container for data relating to the FRSF solver faces.
struct FRSF_Solver_Face_T {
	struct Solver_Face_T face; ///< The base \ref Solver_Face_T.

	// Members required for 2nd order PDE terms.

	/// \brief Container for information relating to volumes on either side of the \ref FRSF_Solver_Face_T.
	struct Neigh_Info_FRSF {
		struct Multiarray_T* grad_coef_f; ///< The face contributions to the solution gradient coefficients.

		/** Linearization of \ref FRSF_Solver_Face_T::Neigh_Info_FRSF::grad_coef_f wrt \ref Solver_Volume_T::sol_coef
	...
	} neigh_info[2]; ///< \ref Neigh_Info_FRSF. Uses the same indexing convention as that of \ref Face::neigh_info.
};

/// \brief Constructor for a derived \ref FRSF_Solver_Face_T.
void constructor_derived_FRSF_Solver_Face_T
	(struct Face* face_ptr,       ///< Pointer to the face.
	 const struct Simulation* sim ///< \ref Simulation.
	);

/// \brief Destructor for a derived \ref FRSF_Solver_Face_T.
void destructor_derived_FRSF_Solver_Face_T
	(struct Face* face_ptr ///< Pointer to the face.
	);
