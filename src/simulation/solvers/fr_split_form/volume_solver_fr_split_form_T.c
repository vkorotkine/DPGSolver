...
void constructor_derived_FRSF_Solver_Volume_T (struct Volume* volume_ptr, const struct Simulation* sim)
{
	...
	struct FRSF_Solver_Volume_T* frsf_s_vol = (struct FRSF_Solver_Volume_T*) volume_ptr;
	...
	if (test_case->has_2nd_order) {
		...
		frsf_s_vol->grad_coef_v = constructor_zero_Multiarray_T('C',order,extents); // destructed

		if (test_case->solver_method_curr == 'i') {
			for (int i = 0; i < DIM; ++i)
				frsf_s_vol->d_g_coef_v__d_s_coef[i] = constructor_empty_const_Matrix_T('R',0,0); // destructed
		} else {
...
void destructor_derived_FRSF_Solver_Volume_T (struct Volume* volume_ptr)
{
	struct FRSF_Solver_Volume_T* frsf_s_vol = (struct FRSF_Solver_Volume_T*) volume_ptr;

	destructor_conditional_Multiarray_T(frsf_s_vol->sol_coef_p);
	destructor_conditional_const_Matrix_T(frsf_s_vol->m_inv);
	destructor_conditional_const_Matrix_T(frsf_s_vol->m);
	destructor_conditional_Multiarray_T(frsf_s_vol->grad_coef_v);
	for (int i = 0; i < DIM; ++i)
		destructor_conditional_const_Matrix_T(frsf_s_vol->d_g_coef_v__d_s_coef[i]);
}
