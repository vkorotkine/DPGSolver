...
void constructor_derived_FRSF_Solver_Face_T (struct Face* face_ptr, const struct Simulation* sim)
{
	const struct Face*const face            = face_ptr;
	struct FRSF_Solver_Face_T*const frsf_s_face = (struct FRSF_Solver_Face_T*) face_ptr;
	...
	if (test_case->has_2nd_order) {
		...
		for (int i = 0; i < n_neigh; ++i) {
			...
			struct Neigh_Info_FRSF*const ni = &frsf_s_face->neigh_info[i];
...
void destructor_derived_FRSF_Solver_Face_T (struct Face* face_ptr)
{
	const struct Face*const face            = face_ptr;
	struct FRSF_Solver_Face_T*const frsf_s_face = (struct FRSF_Solver_Face_T*) face_ptr;
