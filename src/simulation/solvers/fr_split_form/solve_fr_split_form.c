...
#include "compute_source_rlhs_fr_split_form.h"
...
/// \brief Call the common functions when computing the rlhs values for explicit and implicit fr_split_form schemes.
static void compute_rlhs_common_fr_split_form
 ...
 */
static void fill_petsc_Vec_b_fr_split_form
	(const struct Simulation*const sim,       ///< Defined for \ref compute_rlhs_fr_split_form.
	 struct Solver_Storage_Implicit*const ssi ///< Defined for \ref compute_rlhs_fr_split_form.
	);
 ...
 *  \f]
 *  where \f$ M \f$ is \ref FRSF_Solver_Volume_T::m.
 *
...
double compute_rlhs_fr_split_form (const struct Simulation* sim, struct Solver_Storage_Implicit* ssi)
{
	compute_rlhs_common_fr_split_form(sim,ssi);
	compute_CFL_ramping(ssi,sim);
	fill_petsc_Vec_b_fr_split_form(sim,ssi);

	return compute_max_rhs_dg_like(sim);
}

void set_petsc_Mat_row_col_fr_split_form
...
static void compute_rlhs_common_fr_split_form (const struct Simulation*const sim, struct Solver_Storage_Implicit*const ssi)
{
...
static void compute_CFL_ramping (struct Solver_Storage_Implicit*const ssi, const struct Simulation*const sim)
{
	...
	for (struct Intrusive_Link* curr = sim->volumes->first; curr; curr = curr->next) {
		...
		struct FRSF_Solver_Volume* frsf_s_vol = (struct FRSF_Solver_Volume*) curr;

		/// \note See the comments for this function for the justification of the scaling.
		const struct const_Matrix_d*const m_dt = constructor_copy_scale_const_Matrix_d(frsf_s_vol->m,-1.0/dt); // dest.

		for (int eq = 0; eq < n_eq; ++eq) {
			set_petsc_Mat_row_col_fr_split_form(ssi,s_vol,eq,s_vol,eq);
