...
/** \brief Return a statically allocated `char*` holding the specific header for all of the inviscid Burgers variables.
 *  \return See brief. */
static const char* compute_header_spec_burgers_inviscid_all ( );

// Interface functions ********************************************************************************************** //

struct Error_CE* constructor_Error_CE_burgers_inviscid_all (const struct Simulation* sim)
{
	assert(DIM == 1); // Add support.
	const int n_out = DIM;

	struct Error_CE_Helper* e_ce_h = constructor_Error_CE_Helper(sim,n_out);
	e_ce_h->header_spec = compute_header_spec_burgers_inviscid_all();
