...
{
	struct Multiarray_d* sol = constructor_sol_supersonic_vortex(xyz,sim); // returned
...
void set_sol_supersonic_vortex (const struct Simulation* sim, struct Solution_Container sol_cont)
{
	...
	struct Multiarray_d* sol = constructor_sol_supersonic_vortex(xyz,sim); // destructed
	...
	(const struct const_Multiarray_d* xyz, const struct Simulation* sim)
{
	...
	for (int i = 0; i < n_n; ++i) {
		const double r_i   = sol_data.r_i,
		             m_i   = sol_data.m_i,
		             rho_i = sol_data.rho_i,
		             V_i   = sol_data.V_i;

		const double r  = sqrt(x[i]*x[i]+y[i]*y[i]),
		             t  = atan2(y[i],x[i]),
		             Vt = -V_i/r;
