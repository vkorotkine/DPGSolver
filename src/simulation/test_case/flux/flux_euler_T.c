...
void compute_Flux_euler_T (const struct Flux_Input_T* flux_i, struct mutable_Flux_T* flux)
{
	const ptrdiff_t NnTotal = flux_i->s->extents[0];

//	Type const *const W = flux_i->s->data;
	const Type *const W = flux_i->s->data;
...
void compute_Flux_euler_jacobian_T (const struct Flux_Input_T* flux_i, struct mutable_Flux_T* flux)
{
	const ptrdiff_t NnTotal = flux_i->s->extents[0];
