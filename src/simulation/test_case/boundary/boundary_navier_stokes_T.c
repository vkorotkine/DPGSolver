...
typedef const Type* (*compute_uvw_ex_fptr)
	(const Real xyz[DIM],                           ///< xyz-coordinates of the point.
...
static void constructor_Boundary_Value_T_navier_stokes_no_slip_all_general
	(struct Boundary_Value_T*const bv,               ///< See brief.
	 ...
	 Solver_Face_T*const s_face,                     ///< See brief.
	 const struct Exact_Boundary_Data*const eb_data, ///< \ref Exact_Boundary_Data.
...
// read/set data (V_tangential)
	constructor_Boundary_Value_T_navier_stokes_no_slip_all_general(bv,bv_i,face,sim);
}
...
static void constructor_Boundary_Value_T_navier_stokes_no_slip_all_general
	(struct Boundary_Value_T*const bv, const struct Boundary_Value_Input_T*const bv_i, const struct
	 Solver_Face_T*const s_face, const struct Exact_Boundary_Data*const eb_data,
	 struct const struct Simulation*const sim)
{
	EXIT_ADD_SUPPORT; UNUSED(bv); UNUSED(bv_i); UNUSED(face); UNUSED(sim);
...
/** \todo Ensure that the correction of xyz to the exact boundary has been added. see comments in
 *        boundary_diffusion_T.c. */
...
EXIT_ADD_SUPPORT;
	...
	for (int n = 0; n < n_n; n++) {
		const Real xyz_n[] = ARRAY_DIM( get_col_const_Multiarray_R(0,xyz),
		                                get_col_const_Multiarray_R(1,xyz),
		                                get_col_const_Multiarray_R(2,xyz) );
		const Real* uvw_ex = eb_data->compute_uvw_ex(xyz_n,eb_data);
