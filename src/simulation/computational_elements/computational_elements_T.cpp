...
void constructor_derived_computational_elements_T (struct Simulation* sim, const int derived_category)
{
	...
	for (struct Intrusive_Link* curr = sim->volumes->first; curr; ) {
		...
		de_i.cpponstructor_derived_Volume((struct Volume*)curr,sim);
		curr = next;
	}
	...
	for (struct Intrusive_Link* curr = sim->faces->first; curr; ) {
		struct Intrusive_Link* next = curr->next;
		de_i.cpponstructor_derived_Face((struct Face*)curr,sim);
		curr = next;
	}
	...
	(const int derived_category, const struct Simulation* sim)
{
	...
	case IL_SOLVER:
		...
		de_info.cpponstructor_derived_Volume = constructor_derived_Solver_Volume_T;
		de_info.cpponstructor_derived_Face   = constructor_derived_Solver_Face_T;
		break;
	case IL_SOLVER_DG:
		...
		de_info.cpponstructor_derived_Volume = constructor_derived_DG_Solver_Volume_T;
		de_info.cpponstructor_derived_Face   = constructor_derived_DG_Solver_Face_T;
		break;
	case IL_SOLVER_DPG:
		assert(sim->volumes->name == IL_VOLUME_SOLVER);
		assert(sim->faces->name   == IL_FACE_SOLVER);
		...
		de_info.cpponstructor_derived_Volume = constructor_derived_DPG_Solver_Volume_T;
		de_info.cpponstructor_derived_Face   = constructor_derived_DPG_Solver_Face_T;
		break;
	case IL_SOLVER_OPG:
		assert(sim->volumes->name == IL_VOLUME_SOLVER);
		assert(sim->faces->name   == IL_FACE_SOLVER);
		...
		de_info.cpponstructor_derived_Volume = constructor_derived_OPG_Solver_Volume_T;
		de_info.cpponstructor_derived_Face   = constructor_derived_OPG_Solver_Face_T;
		break;
	...
	case IL_SOLVER_ADAPTIVE:
		...
		de_info.cpponstructor_derived_Volume = constructor_derived_Adaptive_Solver_Volume;
		de_info.cpponstructor_derived_Face   = constructor_derived_Adaptive_Solver_Face;
		break;
#endif
	default:
