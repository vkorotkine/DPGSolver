...
/* 
  ...
  Comments:
    ...
    This ordering allows for comparison between facets from different volumes such that they can be matched up in VToV.
  ...
  Notation:
    ...
    NGF : (N)umber of (G)lobal (F)acets
             Note: This number is modified as the connectivity/periodicity is established, and non-existent facets are
    ...
    FToVe  : (F)acet to (Ve)rtices correspondence.
             Note: This potentially includes non-existent facets. In this case, all facet vertices are set equal to
                   NVe, which is not a possible value.
...
//printf("%d %d\n",NGF,NfveMax);
  ...
  for (v = 0; v < NV; v++) {
    ...
    for (f = 0; f < Nf; f++) {
      ...
      else if (type == WEDGE || type == PYR) { // Different FACET types
        if ((type == WEDGE && f < 2) || (type == PYR && f < 4)) FNve[v*NfMax+f] = Nfve[0];
        else                                                    FNve[v*NfMax+f] = Nfve[1];
      }
      else printf("Error: Element type input from gmsh is not supported.\n"), exit(1);
      ...
      if (type != WEDGE && type != PYR) {
        ...
        printf("Update SetupConnectivity to handle WEDGE/PYRAMID elements.\n");
        exit(1);
      }
    }
  }
  ...
  // Note: There are extra non-existent faces at the end of the list which are ignored.
  ...
  for (i = 0; i < IndM; i++) IndicesMatching[i] = IndicesMatchingOver[i];
