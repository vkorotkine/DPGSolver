...
/* 
  ...
  Notation:
    ...
    NodeType()[] : Node type used for each type of node () and each type of element [].
                   () : (S)olution, (F)lux, (F)lux in (r)eference space (s)traight/(c)urved
                        (I)integration (s)traight/(c)urved (f)acet/(v)olume
                   [] : TP [0], SI [1], PYR [2]
...
void setup_parameters()
{
  ...
  if (DB.P > PMax) printf("Error: P must be less than or equal PMax.\n"), exit(1);
  NP  = PMax+1;
  NDE = 3;
  ...
  if      (DB.Restart == -1) DB.PR = 0;
  else if (DB.Restart ==  0) { 
    if (DB.P == 0) printf("Invalid entry for Restart.\n"), exit(1);
    else           DB.PR = DB.P-1;
  }
  else DB.PR = DB.P;

  if (DB.P == 0) DB.PP = DB.P+1;
  else           DB.PP = max(6-ML,DB.P);

  // Geometry
  PGs = 1;

  if (strstr(DB.MeshType,"ToBeCurved") != NULL) DB.AC = 1, DB.ExactGeom = 0;
  else                                          DB.AC = 0, DB.ExactGeom = 1;
  ...
  for (P = 0; P <= PMax; P++) {
    ...
    if (!Collocated) {
      ...
      // Interpolation (TP)
      if (!EFE) PFrs[P][0] = PCs[P][0]+PF[P], PFrc[P][0] = PCc[P][0]+PF[P];
      else      PFrs[P][0] = P,               PFrc[P][0] = P;

      if (strstr(DB.NodeType,"GLL") != NULL) {
        if (P == 0) strcpy(NodeTypeS[P][0],"GL");
        else        strcpy(NodeTypeS[P][0],"GLL");

        if (PF[P] == 0) strcpy(NodeTypeF[P][0],"GL");
        else            strcpy(NodeTypeF[P][0],"GLL");
      ...
      // Interpolation (SI)
      if (!EFE) PFrs[P][1] = PCs[P][1]+PF[P], PFrc[P][1] = PCc[P][1]+PF[P];
      else      PFrs[P][1] = P,               PFrc[P][1] = P;

      if (strstr(DB.NodeType,"AO") != NULL) {
        if (P == 0) {
          if      (d == 2) strcpy(NodeTypeS[P][1],"WS");
          else if (d == 3) strcpy(NodeTypeS[P][1],"SH");
        }
        else               strcpy(NodeTypeS[P][1],"AO");

        if (PF[P] == 0) {
          if      (d == 2) strcpy(NodeTypeF[P][1],"WS");
          else if (d == 3) strcpy(NodeTypeF[P][1],"SH");
        }
        else               strcpy(NodeTypeF[P][1],"AO");
    ...
    if (strstr(DB.BasisType,"Nodal") == NULL) { 
      ...
      printf("Nodal BasisType must be selected to use a collocated scheme\n"); exit(1); 
    }

    // Note: For collocated interpolation and integration nodes, a desired integration order cannot be specified.
    PF[P] = P;
    ...
    if (strstr(DB.NodeType,"GLL") != NULL && P > 0) {
      /* Brian's original code parameters
         This is the only version of the scheme where all nodes are collocated (i.e. FACET nodes are collocated
         with VOLUME nodes as well). Consequently, there is negligible additional cost for the strong form as 
         compared to the weak form as the discontinuous boundary flux term is already evaluated in the VOLUME term.
      */
