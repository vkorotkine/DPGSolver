...
/// \brief Provides unit tests for the NURBS tensor-product basis functions.
static void test_unit_basis_tensor_product_nurbs
	(struct Test_Info*const test_info ///< \ref Test_Info.
	);
	...
	)
{
	...
	else if (strcmp(test_name,"tp_nurbs") == 0)
		test_unit_basis_tensor_product_nurbs(&test_info);
	else
...
/** \brief Constructor for \ref Basis_Data_TP_nurbs.
 *  \return Standard. */
static struct Basis_Data_TP_nurbs* constructor_Basis_Data_TP_nurbs
	(const char eval_type ///< Method to use to obtain the data. Options: 'a'nalytical, 'c'ompute.
	);

/// \brief Destructor for \ref Basis_Data_TP_nurbs.
static void destructor_Basis_Data_TP_nurbs
	(struct Basis_Data_TP_nurbs* b_data ///< Standard.
	);

static void test_unit_basis_tensor_product_nurbs (struct Test_Info*const test_info)
{
	UNUSED(test_info);
	bool    pass        = false;
	double* tol         = NULL;
	...
	struct Basis_Data_TP_nurbs* b_data_a = constructor_Basis_Data_TP_nurbs('a'), // destructed
	                          * b_data_c = constructor_Basis_Data_TP_nurbs('c'); // destructed
	...
	destructor_Basis_Data_TP_nurbs(b_data_a);
	destructor_Basis_Data_TP_nurbs(b_data_c);
...
static struct Basis_Data_TP_nurbs* constructor_Basis_Data_TP_nurbs (const char eval_type)
{
	struct Basis_Data_TP_nurbs* b_data = calloc(1,sizeof *b_data); // returned

	const struct const_Nodes* d1_p4_GLL = constructor_const_Nodes_tp(1,4,NODES_GLL), // destructed
	                        * d2_p4_GLL = constructor_const_Nodes_tp(2,4,NODES_GLL), // destructed
	                        * d3_p4_GLL = constructor_const_Nodes_tp(3,4,NODES_GLL); // destructed
	const char* basis_name = "tp_nurbs";
	if (eval_type == 'a') {
		b_data->phi13        = constructor_basis_tp_nurbs_def(3,d1_p4_GLL->rst);      // keep
		b_data->phi22        = constructor_basis_tp_nurbs_def(2,d2_p4_GLL->rst);      // keep
		b_data->phi32        = constructor_basis_tp_nurbs_def(2,d3_p4_GLL->rst);      // keep
		b_data->grad_phi13   = constructor_grad_basis_tp_nurbs_def(3,d1_p4_GLL->rst); // keep
		b_data->grad_phi22   = constructor_grad_basis_tp_nurbs_def(2,d2_p4_GLL->rst); // keep
		b_data->grad_phi31   = constructor_grad_basis_tp_nurbs_def(1,d3_p4_GLL->rst); // keep
		b_data->p_14         = constructor_part_unity_def(d1_p4_GLL->rst->ext_0);      // keep
		b_data->p_24         = constructor_part_unity_def(d2_p4_GLL->rst->ext_0);      // keep
		b_data->p_34         = constructor_part_unity_def(d3_p4_GLL->rst->ext_0);      // keep
		b_data->grad_coef_13 = constructor_grad_vals_computation_def(1,3,basis_name);  // keep
		...
		b_data->phi13        = constructor_basis_tp_nurbs(3,d1_p4_GLL->rst);      // keep
		b_data->phi22        = constructor_basis_tp_nurbs(2,d2_p4_GLL->rst);      // keep
		b_data->phi32        = constructor_basis_tp_nurbs(2,d3_p4_GLL->rst);      // keep
		b_data->grad_phi13   = constructor_grad_basis_tp_nurbs(3,d1_p4_GLL->rst); // keep
		b_data->grad_phi22   = constructor_grad_basis_tp_nurbs(2,d2_p4_GLL->rst); // keep
		b_data->grad_phi31   = constructor_grad_basis_tp_nurbs(1,d3_p4_GLL->rst); // keep
		b_data->p_14         = constructor_part_unity(b_data->phi13);              // keep
		b_data->p_24         = constructor_part_unity(b_data->phi22);              // keep
		b_data->p_34         = constructor_part_unity(b_data->phi32);              // keep
		b_data->grad_coef_13 = constructor_grad_vals_computation(1,3,basis_name);  // keep
...
static void destructor_Basis_Data_TP_nurbs (struct Basis_Data_TP_nurbs* b_data)
{
	destructor_const_Matrix_d(b_data->phi13);
	destructor_const_Matrix_d(b_data->phi22);
	destructor_const_Matrix_d(b_data->phi32);
