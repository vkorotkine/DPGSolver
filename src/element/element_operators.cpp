	 ...
	 const struct const_Element* element, const struct Simulation* sim)
{
	...
	const_cast_c(&op_info->op_io[OP_IND_I].cppe,  name_in[0]);
	const_cast_c(&op_info->op_io[OP_IND_I].kind,name_in[1]);
	const_cast_c(&op_info->op_io[OP_IND_I].sc,  name_in[2]);

	const_cast_c(&op_info->op_io[OP_IND_O].cppe,  name_out[0]);
...
static void set_current_op_io (const struct Operator_Info* op_info, const int* op_values)
{
	...
	for (int i = 0; i < 2; ++i) {
		const_cast_i(&op_io[i].cppe_op,ce_ptr[i]);
		const_cast_i(&op_io[i].h_op,h_ptr[i]);
		const_cast_i(&op_io[i].p_op,p_ptr[i]);
		const_cast_i(&op_io[i].s_type,compute_super_type_op(op_io[i].cppe,op_io[i].h_op,op_info->element));
	}
}
...
static bool op_should_use_L2 (const int*const op_values, const struct Operator_Info*const op_info)
{
	...
	const char ce_i = op_io[OP_IND_I].cppe,
	           ce_o = op_io[OP_IND_O].cppe;
	 ...
	 const struct const_Multiarray_Matrix_d* op_ioN)
{
	...
	struct Op_IO op_io_C[2] = {
		{.cppe = 'v', .kind = 'c', .sc = sc, .h_op = 0, .p_op = p_op_c, .s_type = s_type_o, },
		{.cppe = 'v', .s_type = s_type_o, },
	};
	...
	struct Op_IO op_io_F[2] = {
		{.cppe = 'v', .kind = 'c', .sc = sc, .h_op = h_i, .p_op = p_op_c, .s_type = s_type_o, },
		{.cppe = 'v', .s_type = s_type_o, },
	};
