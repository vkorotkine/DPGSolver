 ...
 *	not possible to define them! To overcome this difficulty, casts are used to set the data.
 *
 ...
 *	arguments when `order > 2`. Example implementations can be found in questions and answers of [this][SO_compound1]
 *	and [this][SO_compound2] SO discussion.
 *
 *	<!-- References: -->
...
struct Multiarray_Vector_i* constructor_copy_Multiarray_Vector_i_i
	(const int* data_V,              ///< The `int` data.
	 const int*const ext_V,          /**< The ext_0 values of each \ref Vector_i in the multiarray. Note: using `int`
	                                  and not `ptrdiff_t` as this is sometimes initialized from \ref Vector_i::data. */
	 const int order,                ///< Defined in \ref Multiarray_d.
	 const ptrdiff_t*const extents_i ///< The input extents.
	);
...
/**	\brief Move constructor for a \ref Multiarray_Vector_i\* with the input extents having been previously dynamically
 *	       allocated.
 *	\return See brief. */
