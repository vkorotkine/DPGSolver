...
/* 
  ...
  Comments:
    The function achieves the result using the following steps:
      1) Find the number of periodic facets in each direction and store their associated number of vertices so that TRIs
         and QUADs are not matched up.
      ...
      3) Looping over the master facets in each direction, master vertices are compared to slave vertices on each
         possible slave periodic facet using the potential connections array. 
           If a match is found, the slave facet is no longer considered for subsequent master facets.
           Once the loop has terminated, a check is performed to ensure that the number of matches found is equal to the
           number of master periodic facets in the mesh file.
      4) Once all periodic connections have been established, VToV, VToF, VToGF, GFToVe, and GFC are modified to reflect
         the changes.
...
void SetupPeriodic()
{
  ...
  if (IndPFM != IndPFS) { printf("Error: Master and Slave periodic facet mismatch\n"); exit(1); }
  NPFTotal = IndPFM;
...
//ArrayPrinti(1,NPFTotal,PFToGFS);
  ...
  for (dim = 0, IndPF = 0, iStart = 0; dim < d; dim++) {
    if (dim != 0) iStart += NPF[dim-1]*NfveMax;
    for (i = 0; i < NPF[dim]; i++) {
      for (j = 0; j < PFNveM[IndPF]; j++) {
        PFToVeM[iStart+i*NfveMax+j] = GFToVe[PFToGFM[IndPF]*NfveMax+j]; 
      }
      PetscSortInt(PFNveM[IndPF],&PFToVeM[iStart+i*NfveMax]);
      for (j = 0; j < PFNveS[IndPF]; j++) {
        PFToVeS[iStart+i*NfveMax+j] = GFToVe[PFToGFS[IndPF]*NfveMax+j]; 
      }
      PetscSortInt(PFNveS[IndPF],&PFToVeS[iStart+i*NfveMax]);
      IndPF++;
    }
  }
  free(PFToGFM);
...
//ArrayPrinti(NPFSum[d-1],NfveMax,PFToVeS);
  ...
  for (i = 0; i < NPVeUnique*3; i++) PConn[i] = NVe;
  
  for (i = 0, IndPVe = 0; i < NPVeUnique; i++) {
...
//ArrayPrinti(1,NPVeUnique,PVeUnique);
  ...
  for (dim = 0, iStart = 0, IndPFM = 0; dim < d; dim++) {
    ...
    for (i = 0; i < NPF[dim]; i++) PFSFound[i] = 0;
    for (PFM = 0; PFM < NPF[dim]; PFM++) {
      ...
      IndPFS = 0;
      for (i = 0; i < dim; i++) IndPFS += NPF[dim];
      for (PFS = 0; PFS < NPF[dim]; PFS++) {
        if (PFNveS[IndPFS] != NpveM) continue; // ensure that TRIs and QUADs are not paired. 

        for (i = 0; i < NpveM; i++) veS[i] = PFToVeS[IndPFS*NfveMax+i];
        ...
        if (match == 1) {
          ...
          vM = PFVFIndM[IndPFM*2]; fM = PFVFIndM[IndPFM*2+1];
          vS = PFVFIndS[IndPFS*2]; fS = PFVFIndS[IndPFS*2+1];

          //printf("%d %d %d %d \n",vM,fM,vS,fS);
          
          VToV[vM*NfMax+fM] = vS; VToF[vM*NfMax+fM] = fS;
          VToV[vS*NfMax+fS] = vM; VToF[vS*NfMax+fS] = fM;
  ...
  if (IndPFM != NPFSum[d-1]) printf("Error: Did not find the correct number of periodic connections.\n"), exit(1);
