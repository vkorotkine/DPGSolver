...
/* 
  ...
  Comments:
    ...
    Periodic:
      The periodic processing is done in this function (in addition to simply reading the mesh file) because the desired
      output (PVe) cannot simply be read from the file (although this would be the ideal case).
      ...
      Convention: *** IMPORTANT ***
        ...
        format specified below for the definition of Points, Lines, Surfaces, and Volumes in gmsh. This is not required
        when periodicity is not present.
    ...
    Parmetis:
      ...
      Parmetis allows for weight constraints for mesh partitioning. When running using the full NS equations, check if
      more heavily weighting viscous elements results in improved performance; this would likely not be done in the 
      initial mesh distribution but in a redistribution while the code is running. (ToBeDeleted)

      ParMETIS_V3_PartMeshKway: While the manual indicates that part stores the local vertices, in this case, it stores
                                the local elements (VOLUMEs).

      Parmetis - 6) Restrictions & Limitations:
        Restriction 2 is false for the functions which are called here (i.e. Parmetis calls can be made using a single
        processor). There is, in fact, a special condition in the function calling metis if a single processor is used.

    Miscellaneous:

      ToBeDeleted: After profiling the code, investigate using ParMETIS_V3_PartGeomKway if the current routine turns out
                   to be slow. 

      ToBeDeleted: The professor mentioned that the way OpenMPI is implemented here is "blocking", i.e. requires a
                   potentially large amount of wait time because of MPI_Barrier. There is a non-blocking way of using
                   OpenMPI sending the receiver information first. => Investigate later.
  ...
  Notation:
    ...
    EToPrt : (E)lement to (P)a(rt)ition correspondence.
             Note: The partitioning is done in parallel and the GLOBAL EToPrt array for all VOLUMEs is then assembled on
                   each processor using MPI. 
...
void GmshReader()
{
  ...
  FILE   *fID;
    ...
    // Arbitrary initializations for variables defined in conditionals (to eliminate warnings)
  ...
  while (!feof(fID)) {
    ...
    if (strstr(StringRead,"$EndNodes") != NULL) SectionNodes = 0;
    ...
    if (strstr(StringRead,"$Elements") != NULL) {
      fscanf(fID,"%[^\n]\n",StringRead);
      fscanf(fID,"%[^\n]\n",StringRead);
      for (i = 0; i < 4; i++) NE[i] = 0;
      SectionElements = 1;
    }
    if (strstr(StringRead,"$EndElements") != NULL) SectionElements = 0;
    ...
    if (SectionElements) {
      ...
      if (ntags != 2) printf("Error: Too many node tags.\n"), exit(1);
      ...
      for (i = Nve[IndE]; i < 8; i++) EToVe[IndE*8+i] = NVe;
      ...
      else    printf("Unsupported element type read from gmsh file.\n"), exit(1);
    }
        ...
        if (dim == 0) { // Periodic 0D 
          ...
          if ((dimEntered[0] == 0 && dim == 1 && d == 2) || 
            ...
            for (i = 0, IndP = 0; i < NPVe; i++) {
              ...
              if (count == d-1) {
                for (j = 0; j < 2; j++) PVeOverTrue[IndP*2+j] = PVeOver[i*2+j];
                IndP++;
              }
            }
            free(veMatch);
          ...
          // not necessarily list all of the nodes => find them and do not use the node list. (ToBeModified: Move to
          // comments).
          if      (dim == 1) NveMax = 2;
          ...
          for (E = Es[dim], IndS = 0, IndM = 0, IndES = 0, IndEM = 0; E < Es[dim+1]; E++) {
            if (ETags[E*2+1] == TagS) {
              for (j = 0; j < Nve[E]; j++) { 
...
//ArrayPrinti(1,IndM,NodesMOver);
          ...
          for (i = 0; i < NnS; i++) NodesS[i] = NodesSOver[i];
          free(NodesSOver);
...
//ArrayPrinti(NnS,2,PVePossible);
          ...
          for (i = 0, IndPVe = 0; i < NnS; i++) {
            ...
            for (j = IndPVe, Match = 0; j < NPVe; j++) {
              if (PVeOver[j*2+0] == PVePossible[i*2+0] && 
                  PVeOver[j*2+1] == PVePossible[i*2+1]) Match = 1;
            }
...
//ArrayPrinti(NPVe,2,PVeOver);
  ...
  elmdist[0] = 0;
  for (i = 1; i < MPIsize; i++ ) elmdist[i] = elmdist[i-1] + EPerProc;
  ...
  for (i = 0; i < 3; i++) options[i] = 0; // See manual p.17 for options
